#!/usr/bin/env python3
"""航天版报告生成器:把 collect.py 的 JSON 数据用 DeepSeek 生成中文航天日报。

用法: python space_report.py [data.json]
- 优先调 DeepSeek API 生成智能报告
- API 不可用 / 失败则规则化兜底
- 输出到 reports/space_report_YYYY-MM-DD.md
"""
import json
import os
import sys
...
def llm_report(data, date_str):
    ...
    prompt = (
        f"你是资深航天技术编辑。根据下面 {date_str} 收集的航天动态数据(JSON)写一份中文 Markdown 航天日报。\n"
        f"今天日期是 {today},数据里每条有 date、stars 等字段。\n"
        "写作要求:\n"
        "1. **TL;DR**:5 条今日重点,每条一句话。\n"
        "2. **每个项目**:名称 + 一句话简介(基于 summary;**summary 为空时只基于标题写一句并标「(信息有限)」,绝不编造细节**)+ 评分 + 简评。\n"
        "3. **星数显示**:项目标题后用 `⭐数字` 标出 stars(有就标,没有不标)。高星(>1000)或高分(≥8)的项目在简介后加一段深入讲。\n"
        "4. **今日精讲**:综合【评分+星数+创新性+实用性】选 1 个最有未来潜力的,单独一节深入分析:是什么、技术亮点、解决什么问题、未来潜力、潜在风险、与同类对比。\n"
        "5. **日期标注**:date 等于今天标 🆕;不是今天标实际日期。\n"
        "6. 按 **发射任务 / 卫星与星座 / 空间攻防 / 控制与分系统 / 航天前沿与新方法 / 商业与融资** 分类组织(没内容的分类省略)。\n"
        "   - **空间攻防**:技术内容深入讲解,政策/军备动向简单提点即可。\n"
        "   - **控制与分系统**:只涉及卫星(姿轨控、GNC、星敏、反作用轮等),火箭制导控制不涉及。\n"
        "   - **航天前沿与新方法**:新应用领域、AI 结合、MBSE、数字孪生、敏捷研制、组织/研究方法创新都归这里。\n"
        "7. 语言专业简洁,严格基于数据,不编造。每条附链接。知乎/微信公众号来源质量高,可适当侧重。\n"
...
def main():
    ...
    out_path = out_dir / f"space_report_{date_str}.md"
    out_path.write_text(report, encoding="utf-8")
    print(f"[report] 航天报告已生成: {out_path}")
